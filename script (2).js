...
async function fetchSubscriberCount() {
  try {
    ...
    if (count !== undefined) {
      ...
      // Also update by ID for direct targeting
      const subEl = document.getElementById('subCount');
      if (subEl) subEl.textContent = display;
...
(function initReveal() {
  const els = document.querySelectorAll(
    '.feature-card,.song-card,.vibe-card,.headphone-card,.article-card,.contact-card,.newsletter-box'
  );
...
/* ── SEARCH INPUT AUTO-FOCUS WHEN TAG CLICKED ── */
const origQuickSearch = quickSearch;
...
/* ── SMOOTH MOBILE MENU HAMBURGER LINES ── */
